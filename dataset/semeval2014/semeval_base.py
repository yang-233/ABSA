...
def fd(counts):
    '''
    统计频率
    Given a list of occurrences (e.g., [1,1,1,2]), 
    return a dictionary of frequencies (e.g., {1:3, 2:1}.)
    '''
    d = {}
...
按频率降序排列，这是一个函数
Given a map, 
return ranked the keys based on their values.
'''
...
def fd2(counts):
    '''
    统计元组频率
    Given a list of 2-uplets (e.g., [(a,pos), (a,pos), (a,neg), ...]), 
    form a dict of frequencies of specific items (e.g., {a:{pos:2, neg:1}, ...}).
    '''
    d = {}
    for i in counts:
        ...
        if i[0] in d:
            ...
            d[i[0]] = {i[1]: 1} # 每个(i[0] 插入一个字典)
    return d


def validate(filename):
    '''
    Validate an XML file, w.r.t. 
    the format given in the 4th task of **SemEval '14**.
    '''
...
class Category:
    '''
    Category objects contain the term and polarity (i.e., pos, neg, neu, conflict) 
    of the category (e.g., food, price, etc.) of a sentence.
    '''
...
class Aspect:
    '''
    Aspect objects contain the term (e.g., battery life) and 
    polarity (i.e., pos, neg, neu, conflict) of an aspect.
    '''
...
class Instance:
    ...
    def __init__(self, element):
        ...
        self.aspect_terms = [Aspect('', '', offsets={'from': '', 'to': ''}).create(e) 
                            for es in element.findall('aspectTerms') for e in es if es is not None]
        self.aspect_categories = [Category(term='', polarity='').create(e) 
                            for es in element.findall('aspectCategories') for e in es if es is not None]
...
class Corpus:
    '''
    A corpus contains instances, and is useful for 
    training algorithms or splitting to train/test files.
    '''
...
def main(argv=None):
    ...
    if task == 4:
        ...
        print('Accuracy = %f, #Correct/#All: %d/%d' % 
            Evaluate(seen.corpus, predicted).aspect_category_polarity_estimation())
    ...
    if task == 5:
        ...
        print('Task 1: P = %f -- R = %f -- F1 = %f (#correct: %d, #retrieved: %d, #relevant: %d)' % 
            Evaluate(seen.corpus, predicted).aspect_extraction())
        print('Task 2: P = %f -- R = %f -- F1 = %f (#correct: %d, #retrieved: %d, #relevant: %d)' % 
            Evaluate(seen.corpus, predicted).category_detection())
    ...
    if task == 6:
        ...
        print('Task 3: Accuracy = %f (#Correct/#All: %d/%d)' % 
            Evaluate(seen.corpus, predicted).aspect_polarity_estimation())
        print('Task 4: Accuracy = %f (#Correct/#All: %d/%d)' % 
            Evaluate(seen.corpus, predicted).aspect_category_polarity_estimation())
